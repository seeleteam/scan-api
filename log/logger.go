...
func Logger(log *logrus.Logger) gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		if len(c.Errors) > 0 {
			...
			msg := fmt.Sprintf("%s - %s [%s] \"%s %s\" %d %d \"%s\" \"%s\" (%s)", clientIP, hostname, time.Now().Format(timeFormat), c.Request.Method, path, statusCode, dataLength, referer, clientUserAgent, latencyOrigin)
			if statusCode > 499 {
				entry.Error(msg)
